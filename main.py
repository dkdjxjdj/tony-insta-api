...
def api_login():
    ...
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            telegram_app.bot.send_message(chat_id=ADMIN_ID, text=msg),
            loop
        )
    except:
        pass
...
async def show_panel(update: Update):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📢 Toplu Duyuru", callback_data="announce")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "list":
        ...
        for i, u in enumerate(data[-20:], 1):
            text += f"{i}. 👤 {u['user']} | {u['time']}\n"
        text += "\nDetay için kullanıcı adına tıkla."
        ...
        context.user_data["waiting_for"] = "add_ban"
        await query.edit_message_text("⛔ Banlamak istediğin kullanıcı adını yaz:")

    elif cmd == "remove_ban":
        context.user_data["waiting_for"] = "remove_ban"
        ...
        context.user_data["waiting_for"] = "announce"
        await query.edit_message_text("📢 Gönderilecek duyuru mesajını yaz:")
        ...
        with open("tony_logs.txt", "w", encoding="utf-8") as f:
            f.write(txt)
        await context.bot.send_document(chat_id=ADMIN_ID, document=open("tony_logs.txt", "rb"),
                                         filename="tony_logs.txt")
...
async def main():
    ...
    telegram_app.add_handler(CommandHandler("panel", start))
